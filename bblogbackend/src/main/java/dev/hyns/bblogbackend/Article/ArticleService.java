...
public interface ArticleService {
    ...
    default ArticleDTO toDTO(Article entity) {return ArticleDTO.builder().aid(entity.getAid()).title(entity.getTitle()).context(entity.getContext()).createdDate(entity.getCreatedDate()).hide(entity.getHide()).menu(entity.getMenu()).tags(entity.getTags().stream().map(v -> v.getTag()).toList()).visitCnt(entity.getVisits().size()).thumbnail(entity.getThumbnail()).build();}
    default Article toEntity(ArticleDTO dto) {return Article.builder().aid(dto.getAid()).title(dto.getTitle()).context(dto.getContext()).hide(dto.getHide()).menu(dto.getMenu()).tags(dto.getTags().stream().map(v -> Tag.builder().article(Article.builder().aid(dto.getAid()).build()).build()).collect(Collectors.toSet())).thumbnail(dto.getThumbnail()).build();}
}
